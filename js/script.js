 ...
 * 
 * @name script.js
 * @description calculate Celsius, Fahrenheit, Reamur, and Kelvin
 *  
 */
...
buttonConvert.addEventListener("click", () => {
    ...
    if (isNaN(input)) {
        document.getElementById("error").innerHTML = `* Please input only number!`
        return
      // Lakukan pengecekan jika selectOne sama dengan selectTwo maka beri tahu user  
    } else if (selectOne === selectTwo) {
        error.innerHTML = `* The origin and destination units cannot be the same!`
        return
    }
